    ...
    <div className="space-y-6 text-gray-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-[#12141c] border border-[#1e2332] space-y-2">
          ...
          <p className="text-[10px] text-gray-500">Unidades em almoxarifado</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12141c] border border-[#1e2332] space-y-2">
          ...
          <div className="text-3xl font-black text-yellow-400">{lowStockCount}</div>
          <p className="text-[10px] text-gray-500">Necessitam de reposição</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12141c] border border-[#1e2332] space-y-2">
          ...
          <div className="text-3xl font-black text-red-400">{outOfStockCount}</div>
          <p className="text-[10px] text-gray-500">Itens zerados</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#12141c] border border-[#1e2332] space-y-2">
          ...
          <p className="text-[10px] text-gray-500">Ativo imobilizado em produtos</p>
        </div>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-[#12141c] border border-[#1e2332] space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-3 py-1.5 bg-[#181a26] border border-[#2b3145] rounded-xl text-xs text-white"
              />
            </div>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-[#22c55e] hover:bg-[#1eb054] text-black font-extrabold text-xs flex items-center gap-1.5"
          >
              ...
              <tbody className="divide-y divide-[#1d2232]">
                {filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-[#161824] transition-colors">
                    ...
                    <td className="p-3 font-bold text-[#22c55e]">{item.quantity} un</td>
                    <td className="p-3 text-gray-400">{item.minQuantity} un</td>
                    <td className="p-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-[#11131c] border border-[#22c55e]/40 rounded-2xl p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div>
                <label className="block text-gray-300 font-bold mb-1">Nome do Item / Produto</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-gray-300 font-bold mb-1">Quantidade Inicial</label>
                  <input
                    type="number"
                    required
                    value={quantity}
                ...
                <div>
                  <label className="block text-gray-300 font-bold mb-1">Mínimo Alerta</label>
                  <input
                    type="number"
                    value={minQuantity}
              ...
              <div>
                <label className="block text-gray-300 font-bold mb-1">Preço Unitário (R$)</label>
                <input
                  type="number"
                  step="0.01"
                  value={unitPrice}
