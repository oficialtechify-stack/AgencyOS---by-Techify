          ...
          <div className="bg-[#0e0e0e] border border-neutral-800 rounded-3xl p-6 w-full max-w-md shadow-2xl text-neutral-200 relative my-8 animate-scale-up">
            ...
            <form onSubmit={handleUpdateUser} className="space-y-3.5 text-xs">
              ...
              <div className="flex items-center gap-3 p-3 bg-neutral-950 border border-neutral-800 rounded-xl">
                <div className="w-12 h-12 rounded-xl bg-neutral-800 border border-neutral-700 overflow-hidden flex items-center justify-center shrink-0">
                  {editingUser.avatarUrl ? (
                    <img
                      src={editingUser.avatarUrl}
                      alt={editingUser.name || 'Avatar'}
                      className="w-full h-full object-cover"
                      referrerPolicy="no-referrer"
                    />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-[11px] font-bold text-white block">Foto de Perfil</span>
                  <div className="flex items-center gap-2 mt-1">
                    <label className="px-2.5 py-1 bg-neutral-800 hover:bg-neutral-700 text-white rounded-lg text-[10px] font-bold cursor-pointer transition-colors">
                      Alterar Foto
                      <input
                        type="file"
                        accept="image/*"
                        ...
                        onChange={async (e) => {
                          ...
                          if (file) {
                            try {
                              ...
                              setEditingUser({ ...editingUser, avatarUrl: compressed });
                            } catch (err) {
